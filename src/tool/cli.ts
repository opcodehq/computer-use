...
if (values.session && command !== '_serve') {
  try {
    ...
    const args = { ...values, ...(values['input-json'] ? JSON.parse(values['input-json']) : {}),
...
if (command === '_serve') {
  ...
  const verification = { expectedOutput: { type: 'string', minLength: 1, maxLength: 4000, description: 'Optional exact output line to check in non-editable AXStaticText after dispatch. Readback is separate from delivery; alreadyPresent means no causal proof.' } };
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request, extra) => {
    try {
      if (!['desktop_status', 'desktop_apps', 'desktop_installed_apps', 'desktop_launch', 'desktop_observe', 'desktop_act', 'desktop_execute', 'desktop_key', 'desktop_click', 'desktop_type', 'desktop_windows', 'desktop_wait'].includes(request.params.name)) throw new Error('Unknown tool');
  ...
  try {
    const result = await tool.call(command, { ...values, ...(values['input-json'] ? JSON.parse(values['input-json']) : {}), ...(values['window-id'] ? { windowId: Number(values['window-id']) } : {}), ...(values['node-limit'] ? { nodeLimit: Number(values['node-limit']) } : {}), ...(values['dry-run'] === undefined ? {} : { dryRun: values['dry-run'] }) }, controller.signal);
