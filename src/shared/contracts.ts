...
export const ActionSchema = Schema.Struct({
  kind: Schema.Literals(['press', 'focus', 'setValue', 'insertText', 'key', 'click', 'clickElement', 'backgroundClick', 'backgroundKey', 'backgroundText', 'navigate']),
...
export type Event = { state: string; message: string; snapshot?: Snapshot; image?: string; candidate?: Candidate };
export type AppInfo = { pid: number; name: string };
export type Status = { accessibility: boolean; screenRecording: boolean; platform: string; error?: string };
